...
class ClarificationData(ApiData):
    """Create one meeting-owned clarification.

    ``clarification_id`` and ``created_at`` are generated by PostgreSQL. Use
    both ``meeting_id`` and the returned ``clarification_id`` as the CRUD key.
    """
